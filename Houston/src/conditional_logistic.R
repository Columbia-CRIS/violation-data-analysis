...
# label data ----
labeled_data_death <- complete_active_quarters %>% filter(ACTIVE) %>% mutate(
  SEVERE = ifelse(
    NUM_DEATH + NUM_DIS > 0, TRUE, FALSE
  )
)
labeled_data_lost <- complete_active_quarters %>% filter(ACTIVE) %>% mutate(
  SEVERE = ifelse(
...
in_sample_result <- labeled_data_death %>% select(
