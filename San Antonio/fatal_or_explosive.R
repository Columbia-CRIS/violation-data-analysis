...
# This is an updated data generation to separate_fatality.R
# Only days lost caused by "IGNITION OR EXPLOSION OF GAS OR DUST" are recorded
# Inputs: raw accidents data and final data with only active mine-quarters
# Output: days lost, days restrict, deaths, and violations
...
actual.accidents <- accidents %>% filter(
  is.element(inj_degr_desc, c("DAYS AWAY FROM WORK ONLY",
...
# initialize new columns: "deaths" and "perm_dis"
...
# retain days_lost only to rows with "IGNITION OR EXPLOSION OF GAS OR DUST"
...
actual.accidents[which(actual.accidents$ai_class_desc !=  "IGNITION OR EXPLOSION OF GAS OR DUST"), "days_lost"] <- 0
actual.accidents[which(actual.accidents$ai_class_desc !=  "IGNITION OR EXPLOSION OF GAS OR DUST"), "days_restrict"] <- 0
...
save(complete.active.quarters, file="./San Antonio/output/result_select_accidents.RData")
